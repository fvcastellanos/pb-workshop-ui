...
import { WorkOrderHandler } from '@/src/server/handlers/WorkOrderHandler';

const handler = async (accessToken, request, response) => {

    const requestHandler = new WorkOrderHandler(accessToken, request, response);

    switch (request.method) {

        case 'GET':
            await requestHandler.getDetails(request, response);
        break;
        case 'POST':
            await requestHandler.addDetail(request, response);
        break;
        default:
            response.status(405)
