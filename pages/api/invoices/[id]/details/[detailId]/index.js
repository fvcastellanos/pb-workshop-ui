...
import { InvoiceHandler } from '@/src/server/handlers/InvoiceHandler';

const handler = async (accessToken, request, response) => {

    const requestHandler = new InvoiceHandler(accessToken);

    switch (request.method) {

        case 'DELETE':
            await requestHandler.deleteDetail(request, response);
        break;
        case 'PUT':
            await requestHandler.updateDetail(request, response);
        break;
        default:
            response.status(405)
